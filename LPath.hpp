/**
 * @file LPath.hpp
 * @brief std::filesystem::path 的纯头文件薄封装，附带常用路径和目录工具。
 *
 * LPath 的核心定位：
 ...
 * @code
 * LPath src = "src";
 * auto main_cpp = src / "main.cpp";
 *
 * for (auto& cpp : src.recursive_files(".cpp")) {
 *     fmt::println("{}", cpp);
 * }
 * @endcode
 */

#ifndef LPATH_INCLUDE
#define LPATH_INCLUDE
...
#define LPATH_HAS_FILESYSTEM LTOOL_HAS_FILESYSTEM

#if !LPATH_HAS_FILESYSTEM
#error "LPath requires C++17 std::filesystem support"
#endif // !LPATH_HAS_FILESYSTEM

/// list()/glob() 使用的目录项过滤类型。
enum class LPathEntryKind {
    /// 文件、目录、符号链接和其它目录项都保留。
    Any,
    /// 只保留普通文件。
    File,
...
struct formatter<LPath, Char, void> : formatter<std::filesystem::path, Char> {
    template<class FormatContext>
    typename FormatContext::iterator format(const LPath& value, FormatContext& ctx) const;
};
} // namespace fmt

namespace LPathDetail {
...
inline bool matches_kind(const std::filesystem::directory_entry& entry, LPathEntryKind kind) {
    std::error_code ec;
    switch (kind) {
    case LPathEntryKind::File:
        return entry.is_regular_file(ec);
    case LPathEntryKind::Directory:
        return entry.is_directory(ec);
    case LPathEntryKind::Any:
    default:
        return true;
    }
}
 ...
 *
 * LPath 只保存路径本身。成员函数以当前 path_ 为目标，静态函数则适合在不想创建
 * LPath 对象时直接使用。除状态查询外，失败通常抛出 std::runtime_error 或
...
public:
    ...
    /// 移动另一个 LPath。
    LPath(LPath&&) noexcept = default;

    /// 从另一个 LPath 复制赋值。
    LPath& operator=(const LPath&) = default;

    /// 从另一个 LPath 移动赋值。
    LPath& operator=(LPath&&) noexcept = default;

    /// 从 std::filesystem::path 构造。
    LPath(std::filesystem::path path)
        : path_(std::move(path)) {}

    /// 从 C 字符串路径构造；nullptr 视为空路径。
    LPath(const char* path)
        : path_(path ? path : "") {}

    /// 从 std::string 路径构造。
    LPath(const std::string& path)
        : path_(path) {}

    /// 从 LString 路径构造。
    LPath(const LString& path)
        : path_(path.str()) {}

    /// 从 string_view 路径构造。
    LPath(LStringDetail::string_view path)
        : path_(std::string(path)) {}

    /// 从 std::filesystem::path 赋值。
    LPath& operator=(std::filesystem::path path) {
        path_ = std::move(path);
        return *this;
    }

    /// 从 C 字符串路径赋值；nullptr 视为空路径。
    LPath& operator=(const char* path) {
        path_ = path ? path : "";
        return *this;
    }

    /// 从 std::string 路径赋值。
    LPath& operator=(const std::string& path) {
        path_ = path;
        return *this;
    }

    /// 从 LString 路径赋值。
    LPath& operator=(const LString& path) {
        path_ = path.str();
        return *this;
    }
    ...
    LPath& append_path(const std::filesystem::path& rhs) {
        path_ /= rhs;
        return *this;
    }

    /// 返回拼接后的新路径。
    LPath join_path(const std::filesystem::path& rhs) const {
        return path_ / rhs;
    }

    /// 路径拼接运算符，等价于 join_path()。
    friend LPath operator/(const LPath& lhs, const std::filesystem::path& rhs) {
        return lhs.join_path(rhs);
    }

    /// 按路径字面值比较两个 LPath。
    friend bool operator==(const LPath& lhs, const LPath& rhs) noexcept {
        return lhs.path_ == rhs.path_;
    }

    /// 按路径字面值比较 LPath 和 std::filesystem::path。
    friend bool operator==(const LPath& lhs, const std::filesystem::path& rhs) noexcept {
        return lhs.path_ == rhs;
    }

    /// 按路径字面值比较 std::filesystem::path 和 LPath。
    friend bool operator==(const std::filesystem::path& lhs, const LPath& rhs) noexcept {
        return lhs == rhs.path_;
    }

#if __cplusplus >= 202002L
    /// 按 path 的字典序进行三路比较。
    friend auto operator<=>(const LPath&, const LPath&) = default;
#endif // __cplusplus >= 202002L

    /// 将路径字符串写入 ostream。
    friend std::ostream& operator<<(std::ostream& os, const LPath& value) {
        return os << value.path_.string();
    }

    /// 返回 path().filename()。
    LPath filename() const {
    ...
    /// 返回替换扩展名后的副本。
    LPath with_extension(const std::filesystem::path& extension) const {
        auto out = path_;
    ...
    /// 原地替换扩展名，并返回 *this。
    LPath& replace_extension(const std::filesystem::path& extension = std::filesystem::path()) {
        path_.replace_extension(extension);
        return *this;
    }

    /// 返回当前路径相对 @p base 的词法相对路径；不访问真实文件系统。
    LPath lexically_relative(const std::filesystem::path& base) const {
        return path_.lexically_relative(base);
    }

    /// 返回绝对路径。
    LPath absolute() const {
        return std::filesystem::absolute(path_);
    }

    /// 返回规范路径；目标必须存在。
    LPath canonical() const {
        return std::filesystem::canonical(path_);
    }

    /// 返回弱规范路径；适合目标还未完全存在的路径。
    LPath weakly_canonical() const {
        return std::filesystem::weakly_canonical(path_);
    }

    /// 返回当前路径相对 @p base 的文件系统相对路径。
    LPath relative_to(const std::filesystem::path& base = std::filesystem::current_path()) const {
    ...
    LPath& set_last_write_time(file_time_type time) {
        std::filesystem::last_write_time(path_, time);
        return *this;
    }

    /// 创建当前路径指向的目录及其缺失父目录。
    LPath& create_directories() {
        std::filesystem::create_directories(path_);
        return *this;
    }

    /// 创建当前路径指向的单级目录。
    LPath& create_directory() {
        std::filesystem::create_directory(path_);
        return *this;
    }

    /// 创建当前路径的父目录。
    LPath& create_parent_directories() {
        LPathDetail::ensure_parent_directory(path_);
        return *this;
    }
    ...
    /// 将当前路径复制到 @p to。
    const LPath& copy_to(const std::filesystem::path& to, std::filesystem::copy_options options =
    ...
    LPath& move_to(const std::filesystem::path& to) {
    ...
    /// move_to() 的同义函数。
    LPath& rename_to(const std::filesystem::path& to) {
        return move_to(to);
    }

    /// 设置权限。
    LPath& permissions(std::filesystem::perms permissions,
    ...
    /// 列出目录项，可选择递归和过滤隐藏文件名。
    std::vector<LPath> list(LPathEntryKind kind = LPathEntryKind::Any, bool recursive = false,
                            bool include_hidden = true) const {
        return list(path_, kind, recursive, include_hidden);
    }

    /// 列出当前目录下的普通文件。
    std::vector<LPath> files(LStringDetail::string_view extension = {},
                             bool include_hidden = true) const {
        return files(path_, extension, include_hidden);
    }

    /// 递归列出普通文件。
    std::vector<LPath> recursive_files(LStringDetail::string_view extension = {},
                                       bool include_hidden = true) const {
        return recursive_files(path_, extension, include_hidden);
    }

    /// 列出当前目录下的子目录。
    std::vector<LPath> directories(bool include_hidden = true) const {
        return directories(path_, include_hidden);
    }

    /// 使用当前目录作为 base，按文件名执行简单通配匹配。
    std::vector<LPath> glob(LStringDetail::string_view pattern, bool recursive = false,
                            LPathEntryKind kind = LPathEntryKind::Any,
                            bool include_hidden = true) const {
    ...
    /// 返回系统临时目录。
    static LPath temp_directory() {
        return std::filesystem::temp_directory_path();
    }

    /// 生成一个当前尚不存在的临时路径；不会创建文件或目录。
    static LPath temp_path(LStringDetail::string_view prefix = "LPath-",
                           LStringDetail::string_view extension = ".tmp") {
        auto dir = temp_directory().path();
        for (int i = 0; i < 100; ++i) {
            auto name = std::string(prefix) + LPathDetail::unique_token() + std::string(extension);
    ...
    /// 列出目录项，可选择递归和过滤隐藏文件名。
    static std::vector<LPath> list(const std::filesystem::path& directory,
                                   LPathEntryKind kind = LPathEntryKind::Any,
        ...
        if (recursive) {
            for (std::filesystem::recursive_directory_iterator it(directory, options), end;
                 it != end; ++it) {
                if ((!include_hidden && LPathDetail::is_hidden_name(it->path())) ||
                    !LPathDetail::matches_kind(*it, kind)) {
                    continue;
                }
    ...
    /// 列出目录下的普通文件。
    static std::vector<LPath> files(const std::filesystem::path& directory,
                                    LStringDetail::string_view extension = {},
                                    bool include_hidden = true) {
        auto out = list(directory, LPathEntryKind::File, false, include_hidden);
        if (!extension.empty()) {
            out.erase(std::remove_if(out.begin(), out.end(), [&](const LPath& file) {
    ...
    /// 递归列出普通文件。
    static std::vector<LPath> recursive_files(const std::filesystem::path& directory,
                                              LStringDetail::string_view extension = {},
                                              bool include_hidden = true) {
        auto out = list(directory, LPathEntryKind::File, true, include_hidden);
        if (!extension.empty()) {
            out.erase(std::remove_if(out.begin(), out.end(), [&](const LPath& file) {
                return file.extension().str() != extension;
            }), out.end());
    ...
    /// 列出目录下的子目录。
    static std::vector<LPath> directories(const std::filesystem::path& directory,
                                          bool include_hidden = true) {
        return list(directory, LPathEntryKind::Directory, false, include_hidden);
    }
     ...
     */
    static std::vector<LPath> glob_path(const std::filesystem::path& pattern,
                                        bool recursive = false,
                                        LPathEntryKind kind = LPathEntryKind::Any,
                                        bool include_hidden = true) {
        ...
        entries.erase(std::remove_if(entries.begin(), entries.end(), [&](const LPath& file) {
            return !LPathDetail::wildcard_match(file.filename().str(), mask);
        }), entries.end());
        return entries;
    }
...
/// 用户自定义字面量，用于从字符串字面量简洁构造 LPath。
inline LPath operator""_lp(const char* text, std::size_t size) {
    return LPath(std::filesystem::path(std::string(text, size)));
}

/// fmt 格式化器实现：LPath 复用 std::filesystem::path 的格式化规则。
namespace fmt {
template<typename Char>
template<class FormatContext>
typename FormatContext::iterator formatter<LPath, Char, void>::format(
    const LPath& value, FormatContext& ctx) const {
    return formatter<std::filesystem::path, Char>::format(value.path(), ctx);
}
} // namespace fmt

/// std::hash 特化，使 LPath 可用于无序容器。
template<>
struct std::hash<LPath> {
    /// 按 LPath 的底层路径求哈希。
    std::size_t operator()(const LPath& value) const noexcept {
        return value.hash();
    }
};
